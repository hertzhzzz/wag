...
type LegacyInventoryStatus = 'live' | 'gone-410' | 'missing' | 'planned-placeholder' | 'unaccounted'
type PublishingObservedState = 'enabled' | 'disabled' | 'not-observed' | 'unverified'
type PublishingTarget = 'frontend' | 'dashboard' | 'none'
type ApprovalEnforcement = 'technical' | 'policy-only' | 'none' | 'not-applicable'
...
export interface RecommendationEdge {
  mechanism: 'article-prev-next' | 'homepage-top-articles' | 'factory-link-graph'
  sourceRoute: string
  targetRoute: string
...
export interface PublishingPath {
  id: string
  kind: 'generation' | 'scheduling' | 'deployment' | 'notification' | 'tracking'
  source: string
...
function normalizeInternalUrl(rawUrl: string): string | null {
  ...
  } else if (/^https?:\/\//.test(unwrapped) || unwrapped.startsWith('mailto:') || unwrapped.startsWith('tel:')) {
    return null
  }

  if (!pathname.startsWith('/')) return null
  if (/\.(?:avif|gif|jpe?g|pdf|png|svg|webp)(?:[?#].*)?$/i.test(pathname)) return null
...
function extractInternalLinks(content: string): string[] {
  ...
  const linkPattern = /(?:href\s*=\s*["']([^"']+)["']|\]\(([^)\s]+)(?:\s+["'][^"']*["'])?\))/g
...
function collectArticles(projectRoot: string): ArticleInventoryItem[] {
  ...
  return scanMdxFiles(blogDirectory).map((filePath) => {
    ...
    const frontmatterCanonical = typeof data.canonical === 'string' ? data.canonical.trim() : ''
    const frontmatterSlug = typeof data.slug === 'string' && data.slug.trim() ? data.slug.trim() : null
...
function referenceInventoryStatus(
  ...
  if (!slug) return declaredStatus === 'planned' ? 'planned-placeholder' : 'unaccounted'
  if (BLOG_GONE_SLUGS.includes(slug)) return 'gone-410'
  return liveSlugs.has(slug) ? 'live' : 'missing'
}

function collectLegacyClusters(projectRoot: string, articles: ArticleInventoryItem[]): LegacyClusterInventory[] {
    ...
    const references: LegacyClusterReference[] = [
      {
        ...
        inventoryStatus: referenceInventoryStatus(pillarSlug, 'declared-pillar', liveSlugs),
      },
      ...(data.clusters ?? []).map((cluster): LegacyClusterReference => {
        ...
        return {
          ...
          declaredStatus,
          inventoryStatus: referenceInventoryStatus(slug, declaredStatus, liveSlugs),
        }
      }),
    ]
...
const DISCOVERY_SURFACES: DiscoverySurface[] = [
  {
    id: 'article-listing',
    source: 'app/(public)/article/page.tsx and app/components/ArticleListContent.tsx',
    behavior: 'Reads top-level content/blog MDX files, orders known high-impression slugs first, and links each card to /article/{slug}.',
    limitations: 'The listing scan is not recursive and its hard-coded impression order can become stale.',
  },
  {
    id: 'article-detail',
    source: 'app/(public)/article/[slug]/page.tsx and article-utils.ts',
    behavior: 'Recursively scans content/blog, loads the requested MDX file, renders metadata and body content, and supplies previous/next navigation through ArticleNavigation.',
    limitations: 'Seven blocked slugs are excluded by article-utils; frontmatter slug is not the routing source.',
  },
  {
    id: 'sitemap',
    source: 'app/sitemap.ts',
    behavior: 'Recursively scans content/blog and emits /article/{slug} entries after excluding BLOG_GONE_SLUGS.',
    limitations: 'lastModified falls back to 2026-01-01 when an article date is absent.',
  },
  {
    id: 'navigation',
    source: 'app/components/Navbar.tsx, app/components/Footer.tsx, and app/data/nav-links.ts',
    behavior: 'Navbar and footer expose the article index; the shared live service menu also feeds sitemap service URLs.',
    limitations: 'No topic-cluster or pillar navigation is currently exposed in the global navigation.',
  },
  {
    id: 'recommendations',
    source: 'app/(public)/article/[slug]/ArticleNavigation.tsx; app/(public)/article/[slug]/RecommendedSidebar.tsx; app/components/BlogPreview.tsx; app/factory/[slug]/page.tsx; data/link-graph.json',
    behavior: 'Article detail pages expose previous/next links in the recursive filesystem order returned by article-utils. Homepage recommendations use a hard-coded TOP_SLUGS list. Factory pages use explicit link-graph relationships. The same-category RecommendedSidebar helper exists but is not mounted in the article page.',
    limitations: 'Recommendation behavior is split across three mechanisms; previous/next order is filesystem-dependent, the dormant sidebar is not an active discovery surface, and no shared service-led topic graph exists yet.',
  },
  {
    id: 'dashboard',
    source: '../productivity/growth-dashboard/ and ../productivity/daily-analytics/reports/',
    behavior: 'Local growth-dashboard data and dated analytics exports expose article performance for internal review.',
    limitations: 'The dashboard is outside the frontend Git repository and daily_report.py mutates data and may deploy Cloudflare Pages, so this audit reads dated exports only.',
  },
]
...
const MEASUREMENTS: MeasurementBaseline[] = [
  {
    ...
    baseline: 'Last 28 days: 182 clicks, 14,711 impressions, 1.2% CTR. /article/verify-chinese-supplier: 4 clicks, 212 impressions, average position 8.5.',
    source: '../productivity/daily-analytics/reports/2026-07-14.md',
    limitations: 'Static export, not a live query; the report still includes legacy 410 URLs among top pages.',
  },
  {
    id: 'ga4',
    asOf: '2026-07-14',
    availability: 'available',
    baseline: 'GA4 property 526384627, last 28 days Organic Search: 250 sessions, 180 users, and 4 form submissions.',
    source: '../productivity/daily-analytics/reports/2026-07-14.md',
    limitations: 'Static export; it does not provide durable article-to-lead attribution or qualified-lead outcome data.',
  },
  {
    id: 'enquiry',
    asOf: '2026-07-14',
    availability: 'partial',
    baseline: 'Four Organic Search form submissions are visible in the latest GA4 export.',
    source: '../productivity/daily-analytics/reports/2026-07-14.md and current GA4 form_submit tracking',
    limitations: 'No approved 90-day attribution store, qualification status, or privacy-safe article-to-enquiry contract is available yet.',
  },
  {
    id: 'indexation',
    asOf: '2026-07-16',
    availability: 'partial',
    baseline: '/article/check-chinese-company-samr is live and awaiting first-crawl/indexation monitoring through sitemap, internal links, and GSC URL Inspection.',
    source: '../productivity/TASKS.md and ../Handoff.md',
    limitations: 'Historical Indexing API and IndexNow submissions are notifications, not proof of crawl or indexation; no live URL Inspection export is stored for this audit.',
  },
  {
    id: 'geo',
    asOf: AUDIT_DATE,
    availability: 'unavailable',
    baseline: 'No reproducible answer-engine visibility baseline has been captured.',
    source: 'docs/superpowers/specs/2026-07-17-seo-growth-system-design.md',
    limitations: 'The approved 50-question baseline and citation checks are implemented by later tickets 31-36.',
  },
]
...
const SCHEDULER_SNAPSHOT_PATH = 'content/seo/evidence/2026-07-17-scheduler-state.yaml'

const SCHEDULED_TASK_DEFINITIONS = {
  'seo-purge-remaining-410': '~/Claude/Scheduled/seo-purge-remaining-410/SKILL.md',
  'seo-reindex-updated-pages': '~/Claude/Scheduled/seo-reindex-updated-pages/SKILL.md',
  'wag-daily-analytics': '~/Claude/Scheduled/wag-daily-analytics/SKILL.md',
  'wag-google-ads-daily-insight': '~/Claude/Scheduled/wag-google-ads-daily-insight/SKILL.md',
} as const
...
const SCHEDULED_TASK_IDS = Object.keys(SCHEDULED_TASK_DEFINITIONS).sort(compareCodePoints) as ScheduledTaskId[]
...
function publishingPath(
  ...
  return {
    ...
    technicalControl: input.technicalControl ?? {
      ...
      mechanism: 'none',
      evidence: 'No production publication capability is present for this path.',
    },
  }
}
...
function readScheduledTaskSnapshot(projectRoot: string): {
  ...
  const snapshot = parse(fs.readFileSync(snapshotPath, 'utf8')) as ScheduledTaskSnapshot
  if (snapshot.auditDate !== AUDIT_DATE) {
    throw new Error(`${SCHEDULER_SNAPSHOT_PATH} must use auditDate ${AUDIT_DATE}.`)
  }

  const tasks = snapshot.scheduledTasks ?? []
  const taskIds = tasks.map((task) => task.id)
  if (new Set(taskIds).size !== taskIds.length) {
    throw new Error(`${SCHEDULER_SNAPSHOT_PATH} contains duplicate scheduled task IDs.`)
  }
  if (snapshot.taskCount !== tasks.length) {
    throw new Error(`${SCHEDULER_SNAPSHOT_PATH} taskCount must equal scheduledTasks.length.`)
  }
...
function verifySchedulerSnapshotStructure(projectRoot: string): string[] {
  const snapshotPath = path.join(projectRoot, SCHEDULER_SNAPSHOT_PATH)
  if (!fs.existsSync(snapshotPath)) return [`${SCHEDULER_SNAPSHOT_PATH} is missing.`]
  ...
  if (!snapshot.capturedAt || Number.isNaN(new Date(snapshot.capturedAt).getTime())) {
    errors.push(`${SCHEDULER_SNAPSHOT_PATH} capturedAt must be a valid ISO timestamp.`)
  }
  if (snapshot.taskCount !== tasks.length) {
    errors.push(`${SCHEDULER_SNAPSHOT_PATH} taskCount must equal scheduledTasks.length.`)
  }
  if (actualIds.size !== taskIds.length) {
    errors.push(`${SCHEDULER_SNAPSHOT_PATH} contains duplicate scheduled task IDs.`)
  }
  for (const id of taskIds.filter((id) => !SCHEDULED_TASK_IDS.includes(id as ScheduledTaskId)).sort(compareCodePoints)) {
    errors.push(`Unknown scheduler task requires explicit classification: ${id}.`)
  }
  ...
  for (const task of tasks) {
    const expectedDefinition = SCHEDULED_TASK_DEFINITIONS[task.id as ScheduledTaskId]
    ...
    if (!/^[a-f0-9]{64}$/.test(task.definitionSha256 ?? '')) {
      errors.push(`${task.id} definitionSha256 must be a lowercase SHA-256 digest.`)
    }
  }
...
export function verifySchedulerSnapshotBinding(projectRoot: string): string[] {
  ...
  for (const task of tasks.sort((left, right) => compareCodePoints(left.id, right.id))) {
    const expectedDefinition = SCHEDULED_TASK_DEFINITIONS[task.id as ScheduledTaskId]
    if (!expectedDefinition) continue
    ...
    if (task.definitionSha256 !== currentHash) {
      errors.push(`${task.id} definition hash does not match the captured snapshot.`)
    }
  }
...
function resolveLiveSchedulerRegistryPath(options: SchedulerReleaseSafetyOptions): {
  ...
  if (explicitPath) {
    ...
    return fs.existsSync(registryPath)
      ...
      : {
          registryPath: null,
          errors: [`Live scheduler registry is missing at ${toPosixPath(registryPath)}.`],
        }
  }
  ...
  if (!homeRoot) {
    return {
      registryPath: null,
      errors: ['HOME is unavailable; the canonical live scheduler registry cannot be verified.'],
    }
  }
  ...
  const candidates = findFilesNamed(registryRoot, 'scheduled-tasks.json').sort(compareCodePoints)
  if (candidates.length === 1) return { registryPath: candidates[0], errors: [] }
  if (candidates.length === 0) {
    return {
      registryPath: null,
      errors: [`Live scheduler registry was not found under ${toPosixPath(registryRoot)}.`],
    }
  }

  return {
    registryPath: null,
    errors: [`Multiple live scheduler registries were found under ${toPosixPath(registryRoot)} (${candidates.length} candidates).`],
  }
}

function normalizeSchedulerDefinitionPath(filePath: string | undefined): string | undefined {
  if (!filePath) return undefined
  if (filePath === '~' || filePath.startsWith('~/')) return toPosixPath(filePath)
...
function verifyLiveSchedulerRegistryBinding(
  ...
  try {
    liveRegistry = JSON.parse(fs.readFileSync(resolvedRegistry.registryPath, 'utf8')) as ScheduledTaskRegistry
  } catch (error) {
  ...
  const snapshot = parse(fs.readFileSync(snapshotPath, 'utf8')) as ScheduledTaskSnapshot
  const snapshotTasks = new Map((snapshot.scheduledTasks ?? []).map((task) => [task.id, task]))
  ...
  for (const id of liveTaskIds.filter((id) => !SCHEDULED_TASK_IDS.includes(id as ScheduledTaskId))) {
  ...
  for (const id of SCHEDULED_TASK_IDS.filter((id) => !liveTaskMap.has(id))) {
    errors.push(`Expected task is missing from the live scheduler registry: ${id}.`)
  }
  ...
  for (const id of SCHEDULED_TASK_IDS) {
    ...
    if (capturedTask.enabled !== liveTask.enabled) {
      errors.push(`${id} enabled state differs between the snapshot and live scheduler registry.`)
    }
    if (capturedTask.cronExpression !== liveTask.cronExpression) {
      errors.push(`${id} cronExpression differs between the snapshot and live scheduler registry.`)
    }
    if (capturedTask.fireAt !== liveTask.fireAt) {
      errors.push(`${id} fireAt differs between the snapshot and live scheduler registry.`)
    }
    if (capturedTask.filePath !== normalizeSchedulerDefinitionPath(liveTask.filePath)) {
      errors.push(`${id} filePath differs between the snapshot and live scheduler registry.`)
    }
  }
...
export function verifySchedulerSnapshotReleaseSafety(
  ...
  if (!capturedAt || Number.isNaN(capturedAt.getTime())) {
    ...
    if (ageMilliseconds < 0 || ageMilliseconds > maxAgeHours * 60 * 60 * 1000) {
      errors.push(`${SCHEDULER_SNAPSHOT_PATH} is outside the ${maxAgeHours}-hour release-safety window.`)
    }
  }
...
function buildPublishingPaths(projectRoot: string): { paths: PublishingPath[]; schedulerEvidence: string[] } {
  const vercelConfigPath = path.join(projectRoot, 'vercel.json')
  const vercelConfig = fs.existsSync(vercelConfigPath)
    ? JSON.parse(fs.readFileSync(vercelConfigPath, 'utf8')) as { git?: { deploymentEnabled?: boolean } }
    : {}
  ...
  const monorepoAgentSkillInspection = inspectMonorepoAgentSkillPublishingEvidence(projectRoot)
  const scheduledDefinition = (id: keyof typeof SCHEDULED_TASK_DEFINITIONS) => SCHEDULED_TASK_DEFINITIONS[id]
  const scheduledState = (id: keyof typeof SCHEDULED_TASK_DEFINITIONS): PublishingObservedState => {
  ...
  const schedulerEvidence = [
    ...
    ...Object.keys(SCHEDULED_TASK_DEFINITIONS).sort(compareCodePoints).map((id) => {
      const task = scheduler.tasks.get(id)
      if (!task) return `${id}: not present in the discovered registry.`
      const cadence = task.cronExpression ? `cron ${task.cronExpression}` : `one-time fireAt ${task.fireAt ?? 'unknown'}`
  ...
  return {
    ...
    paths: [
      publishingPath({
        ...
        hardBlocker: 'The registered npm scripts contain validation and local generation commands, but no production publish or deploy command.',
        evidenceFiles: [
          'package.json',
      ...
      publishingPath({
        ...
        initiatesPublication: false,
        canPublishProduction: false,
        hardBlocker: 'The file contains detection signatures for read-only repository inspection and has no publication side effect.',
        evidenceFiles: [
      ...
      publishingPath({
        ...
        initiatesPublication: false,
        canPublishProduction: false,
        hardBlocker: 'Direct invocation can mutate canonical MDX content, but it cannot commit, push, or deploy.',
      ...
      publishingPath({
        ...
        initiatesPublication: false,
        canPublishProduction: false,
        hardBlocker: 'The generate branch is a TODO and performs no content generation or publication.',
      ...
      publishingPath({
        ...
        initiatesPublication: false,
        canPublishProduction: false,
        hardBlocker: 'The schedule branch is a TODO and has no runnable scheduler implementation.',
        evidenceFiles: ['scripts/seo_workflow.py'],
      }),
      publishingPath({
        ...
        initiatesPublication: false,
        canPublishProduction: false,
        hardBlocker: 'The examples target an obsolete repository path, the schedule implementation remains a TODO, and the dated scheduler snapshot found no installed cron or LaunchAgent entry.',
        evidenceFiles: ['scripts/README.md', 'scripts/seo_workflow.py', scheduler.path],
        technicalControl: {
          state: 'enforced',
          mechanism: 'non-executable-obsolete-scheduling-example',
          evidence: 'The documented launchd and cron commands cannot reach a runnable publishing implementation.',
        },
      }),
      publishingPath({
        ...
        initiatesPublication: false,
        canPublishProduction: false,
        hardBlocker: 'Both files are non-executable documentation, and the dated scheduler snapshot plus repository scan found no installed scheduler or unattended publisher for this path.',
        evidenceFiles: [
        ...
        technicalControl: {
          ...
          evidence: 'The files are documentation only; no executable launcher for this path was found, and Vercel Git deployments are disabled in vercel.json.',
        },
      }),
      publishingPath({
        ...
        initiatesPublication: false,
        canPublishProduction: false,
        hardBlocker: 'The referenced publisher.publish implementation is absent, so the documented path cannot execute.',
        evidenceFiles: ['../.agents/skills/WAG_seo/SKILL.md'],
        technicalControl: {
          state: 'enforced',
          mechanism: 'missing-publisher-implementation',
          evidence: '../.agents/skills/WAG_seo/SKILL.md references publisher.publish, which is not present in the repository.',
        },
      }),
      publishingPath({
        ...
        initiatesPublication: false,
        canPublishProduction: false,
        hardBlocker: 'Agent skill files are non-executable guidance; their Git push commands require an explicit operator or agent invocation, and Vercel Git deployments are disabled.',
        evidenceFiles: monorepoAgentSkillInspection.evidence,
        technicalControl: {
          state: 'enforced',
          mechanism: 'non-executable-guidance-and-vercel-git-disabled',
          evidence: 'Publishing-capable skill guidance is fully inventoried, and vercel.json disables Git-triggered deployments.',
        },
      }),
      publishingPath({
        ...
        initiatesPublication: false,
        canPublishProduction: false,
        hardBlocker: 'A push can update GitHub, but Vercel Git deployments are disabled in vercel.json.',
        evidenceFiles: ['CLAUDE.md', 'vercel.json'],
        technicalControl: {
          state: gitDeploymentsDisabled ? 'enforced' : 'not-enforced',
          mechanism: gitDeploymentsDisabled ? 'vercel-git-deployments-disabled' : 'none',
          evidence: gitDeploymentsDisabled ? 'vercel.json::git.deploymentEnabled=false' : 'vercel.json does not disable Git deployments.',
        },
      }),
      publishingPath({
      ...
      publishingPath({
        ...
        hardBlocker: 'No unattended repository launcher was observed. Production publication still requires an explicit authenticated manual Vercel command; a technical dual-approval gate remains a Ticket 38 blocker.',
        evidenceFiles: [
          'CLAUDE.md',
          'vercel.json',
        ],
        technicalControl: {
          ...
          evidence: 'Vercel CLI and REST credentials can bypass repository build hooks; the current boundary is manual invocation plus the absence of an unattended launcher.',
        },
      }),
      publishingPath({
      ...
      publishingPath({
        ...
        initiatesPublication: true,
        canPublishProduction: true,
        hardBlocker: 'Direct invocation may deploy the separate Cloudflare dashboard; it cannot publish frontend content.',
        evidenceFiles: [
        ...
        technicalControl: {
          ...
          evidence: 'The script targets productivity/growth-dashboard, not frontend or Vercel.',
        },
      }),
      publishingPath({
        ...
        initiatesPublication: true,
        canPublishProduction: scheduledState('wag-daily-analytics') === 'enabled',
        hardBlocker: 'The enabled task refreshes and deploys the separate growth dashboard, not the frontend.',
        evidenceFiles: [
        ...
        technicalControl: {
          state: 'not-enforced',
          mechanism: 'dashboard-only-boundary',
          evidence: 'The scheduled definition calls productivity/growth-dashboard/deploy-cloudflare.sh.',
        },
      }),
      publishingPath({
        ...
        approvalEnforcement: 'none',
        initiatesPublication: true,
        canPublishProduction: scheduledState('wag-google-ads-daily-insight') === 'enabled',
        hardBlocker: 'The enabled task deploys Ads data to the separate growth dashboard, not the frontend.',
        evidenceFiles: [
      ...
      publishingPath({
        ...
        initiatesPublication: false,
        canPublishProduction: false,
        hardBlocker: 'The one-time notification task is disabled and cannot publish a page.',
        evidenceFiles: [
        ...
        technicalControl: {
          state: scheduledState('seo-purge-remaining-410') === 'disabled' ? 'enforced' : 'not-enforced',
      ...
      publishingPath({
        ...
        technicalControl: {
          state: scheduledState('seo-reindex-updated-pages') === 'disabled' ? 'enforced' : 'not-enforced',
          mechanism: 'scheduler-disabled',
      ...
      publishingPath({
        ...
        initiatesPublication: true,
        canPublishProduction: true,
        hardBlocker: 'Direct shell invocation deploys only the separate growth dashboard.',
        evidenceFiles: [
        ...
        technicalControl: {
          state: 'not-enforced',
          mechanism: 'dashboard-only-boundary',
          evidence: 'Cloudflare Pages project is separate from the Vercel frontend.',
        },
      }),
      publishingPath({
        ...
        initiatesPublication: false,
        canPublishProduction: false,
        hardBlocker: 'The script prepares a local deploy directory and performs no network upload.',
        evidenceFiles: ['../productivity/growth-dashboard/make-deploy.sh'],
      }),
      publishingPath({
        id: 'cloudflare-dashboard-rest-fallback',
        kind: 'deployment',
        source: '../productivity/growth-dashboard/COORDINATION.md::Cloudflare Pages Direct Upload REST API',
        classification: 'manual',
        ...
        initiatesPublication: true,
        canPublishProduction: true,
        hardBlocker: 'The credentialed REST fallback can publish only the separate dashboard.',
        evidenceFiles: ['../productivity/growth-dashboard/COORDINATION.md'],
        technicalControl: {
          state: 'not-enforced',
          mechanism: 'dashboard-only-boundary',
          evidence: 'The documented endpoint targets the Cloudflare growth-dashboard project.',
        },
      }),
      publishingPath({
      ...
      publishingPath({
        ...
        initiatesPublication: false,
        canPublishProduction: false,
        hardBlocker: 'The route can submit Bing notifications but cannot publish content; notification is not indexation proof.',
        evidenceFiles: [
      ...
      publishingPath({
        ...
        initiatesPublication: false,
        canPublishProduction: false,
        hardBlocker: 'The script sends a notification and cannot publish a page.',
        evidenceFiles: [
      ...
      publishingPath({
        ...
        initiatesPublication: false,
        canPublishProduction: false,
        hardBlocker: 'The batch script sends notifications and cannot publish a page.',
        evidenceFiles: [
      ...
      publishingPath({
        ...
        initiatesPublication: false,
        canPublishProduction: false,
        hardBlocker: 'The script submits URL_DELETED notifications and cannot publish content.',
        evidenceFiles: [
      ...
      publishingPath({
        ...
        initiatesPublication: false,
        canPublishProduction: false,
        hardBlocker: 'The helper performs read-only URL Inspection and writes local results.',
      ...
      publishingPath({
        ...
        initiatesPublication: false,
        canPublishProduction: false,
        hardBlocker: 'The SOP is historical documentation and no referenced unattended publisher is installed.',
        evidenceFiles: ['../productivity/WAG-Content-Automation-SOP.md'],
        technicalControl: {
          state: 'enforced',
          mechanism: 'non-executable-historical-documentation',
          evidence: 'The file is documentation only and the current frontend Git deployment path is disabled.',
        },
      }),
    ],
  }
...
function listFiles(directory: string): string[] {
  if (!fs.existsSync(directory)) return []
  return fs.readdirSync(directory, { withFileTypes: true }).flatMap((entry) => {
    if (entry.isDirectory() && ['.git', '.next', '.scratch', '.worktrees', '__pycache__', 'coverage', 'logs', 'node_modules', 'worktrees'].includes(entry.name)) return []
    const fullPath = path.join(directory, entry.name)
    if (entry.isDirectory()) return listFiles(fullPath)
    return entry.isFile() && !/\.(?:pyc|pyo)$/.test(entry.name) ? [fullPath] : []
  })
}

const MAX_PUBLISHING_SCAN_FILE_BYTES = 2 * 1024 * 1024
const OPERATIONAL_NAME_PATTERN = /(blog_generator|seo_workflow|publish|deploy|release|schedul|indexing|indexnow|gsc-batch-check|pipeline-db)/i
const OPERATIONAL_SOURCE_EXTENSIONS = new Set([
  '.bash', '.bat', '.c', '.cjs', '.cmd', '.cpp', '.cs', '.cts', '.fish', '.go', '.java', '.js',
  '.jsx', '.kt', '.lua', '.mjs', '.mts', '.php', '.pl', '.ps1', '.py', '.rb', '.rs', '.sh', '.swift',
  '.ts', '.tsx', '.zsh',
])
const REFERENCED_ENTRYPOINT_PATTERN = /(?:~\/|\/|\.\.?\/)?[A-Za-z0-9_@.+*/\\-]+\.(?:bash|bat|cjs|cmd|cts|fish|js|jsx|mjs|mts|php|pl|ps1|py|rb|sh|ts|tsx|zsh)\b/g
const RUNTIME_EXECUTABLE_PATTERN = /^(?:node(?:js)?|tsx|ts-node|bun|python(?:\d+(?:\.\d+)*)?|bash|sh|zsh|ruby|php|deno)$/i
...
function detectPublishingCapabilities(content: string): PublishingCapability[] {
  ...
  if (cloudflarePagesPattern.test(content)) capabilities.push('cloudflare-pages-deploy')
  if (/\bgit\s+push\b/i.test(content)) capabilities.push('git-push')
  if (
    /(?:https?:\/\/)?(?:api\.indexnow\.org|www\.bing\.com)\/indexnow\b/i.test(content)
...
function readLikelyTextFile(filePath: string): string | null {
  ...
  if (!stats.isFile() || stats.size > MAX_PUBLISHING_SCAN_FILE_BYTES) return null
  ...
  for (const [index, character] of [...normalizedSource].entries()) {
    if (redirectionMode !== null) {
      ...
      if (redirectionQuote === 'single') {
        ...
        else if (redirectionMode === 'input-file') redirectionTarget += character
        continue
      }
...
function readDollarCommandSubstitution(source: string, startIndex: number): { payload: string; endIndex: number } | null {
...
function readBacktickCommandSubstitution(source: string, startIndex: number): { payload: string; endIndex: number } | null {
  let escaped = false
...
function extractWorkflowRunCommands(source: string): { commands: string[]; parseError: string | null } {
...
function extractRuntimeEntrypointCandidates(source: string, depth = 0): string[] {
  if (depth > MAX_SHELL_COMMAND_DEPTH) return []

  const candidates = extractShellCommandSubstitutionPayloads(source)
    .flatMap((payload) => extractRuntimeEntrypointCandidates(payload, depth + 1))

  for (const tokens of tokenizeShellCommandSegments(source)) {
    for (const [runtimeIndex, runtimeToken] of tokens.entries()) {
      const runtimeName = (runtimeToken.replace(/\\/g, '/').split('/').at(-1) ?? '').toLowerCase()
      ...
      if (SHELL_COMMAND_RUNTIMES.has(runtimeName)) {
        for (const [argumentIndex, argument] of runtimeArguments.entries()) {
          if (argument !== '--command' && !/^-[A-Za-z]*c[A-Za-z]*$/.test(argument)) continue
          const payloadIndex = runtimeArguments.findIndex((candidate, candidateIndex) => (
          ...
          candidates.push(...extractRuntimeEntrypointCandidates(runtimeArguments[payloadIndex], depth + 1))
        }
      }
      ...
      for (const [argumentIndex, argument] of runtimeArguments.entries()) {
        ...
        if ((runtimeName === 'bun' || runtimeName === 'deno') && argumentIndex === 0 && argument === 'run') continue
...
function extractReferencedEntrypoints(
  ...
  const resolved = matches.flatMap((rawMatch) => {
    ...
    const possiblePaths = candidate === '~' || candidate.startsWith('~/')
      ? [resolveEvidencePath(candidate)]
      : path.isAbsolute(candidate)
        ? [candidate]
        : [path.resolve(projectRoot, candidate), path.resolve(baseDirectory, candidate)]

    return possiblePaths.filter((filePath) => fs.existsSync(filePath) && fs.statSync(filePath).isFile())
  })
...
function inspectMonorepoAgentSkillPublishingEvidence(projectRoot: string): {
  ...
  for (const filePath of listFiles(skillsRoot)) {
    ...
    if (content === null) {
      ...
      blockers.push(`${normalizeCandidatePath(projectRoot, filePath)}::unscannable-operational-file`)
      continue
    }
    ...
    evidence.push(normalizeCandidatePath(projectRoot, filePath))
    evidence.push(...capabilities.map((capability) => publishingCapabilityToken(projectRoot, filePath, capability)))
  }
...
function canContainExecutablePublishingCapability(
  ...
  return OPERATIONAL_SOURCE_EXTENSIONS.has(extension)
...
function collectPublishingCandidates(projectRoot: string): string[] {
  ...
  const addOperationalCandidate = (filePath: string, content: string) => {
    const capabilities = canContainExecutablePublishingCapability(projectRoot, filePath, content)
      ? detectPublishingCapabilities(content)
      : []
    if (capabilities.length > 0) {
      projectFileCapabilities.set(normalizeCandidatePath(projectRoot, filePath), capabilities)
    }
    if (OPERATIONAL_NAME_PATTERN.test(path.basename(filePath)) || capabilities.length > 0) candidates.push(filePath)
    semanticCandidates.push(...capabilities.map((capability) => publishingCapabilityToken(projectRoot, filePath, capability)))
  }

  const inspectReferencedEntrypoints = (source: string, baseDirectory: string) => {
    const referencedPaths = extractReferencedEntrypoints(projectRoot, source, baseDirectory)
    const capabilities: PublishingCapability[] = []
    ...
    for (const filePath of referencedPaths) {
      ...
      if (content === null) {
        ...
        semanticCandidates.push(`${normalizeCandidatePath(projectRoot, filePath)}::unscannable-referenced-entrypoint`)
        continue
      }
  ...
  for (const filePath of listFiles(projectRoot)) {
    ...
    if (content === null) {
      if (canContainExecutablePublishingCapability(projectRoot, filePath, '')) {
        semanticCandidates.push(`${normalizeCandidatePath(projectRoot, filePath)}::unscannable-operational-file`)
      }
      continue
    }
    addOperationalCandidate(filePath, content)
  ...
  for (const filePath of listFiles(path.join(projectRoot, '.scratch')).filter((candidate) => TEST_OR_SPEC_SOURCE_PATTERN.test(candidate))) {
    const content = readLikelyTextFile(filePath)
    if (content === null) {
      semanticCandidates.push(`${normalizeCandidatePath(projectRoot, filePath)}::unscannable-operational-file`)
      continue
  ...
  for (const filePath of listFiles(path.join(projectRoot, '.github/workflows'))) {
    ...
    if (workflowCommands.parseError) {
      semanticCandidates.push(`${normalizeCandidatePath(projectRoot, filePath)}::unparseable-workflow`)
    }
    const executableSource = [content, ...workflowCommands.commands].join('\n')
    const referenced = inspectReferencedEntrypoints(executableSource, path.dirname(filePath))
  ...
  if (fs.existsSync(packageJsonPath)) {
    ...
    for (const [name, command] of Object.entries(packageJson.scripts ?? {}).sort(([left], [right]) => compareCodePoints(left, right))) {
      const referenced = inspectReferencedEntrypoints(command, projectRoot)
      const previouslyDiscoveredCapabilities = [...projectFileCapabilities.entries()]
      ...
      semanticCandidates.push(...capabilities.map((capability) => `package.json::scripts.${name}::capability.${capability}`))
      semanticCandidates.push(...referencedCapabilities.map((capability) => `package.json::capability.${capability}`))
    }
  }
  ...
  addIfExists(path.join(projectRoot, 'docs/superpowers/plans/2026-05-01-WAG-SEO-文章自动流水线实施计划.md'))
  addIfExists(path.join(projectRoot, 'docs/superpowers/specs/2026-05-01-WAG-SEO-文章自动流水线设计.md'))

  const monorepoRoot = path.dirname(projectRoot)
  const monorepoAgentSkillInspection = inspectMonorepoAgentSkillPublishingEvidence(projectRoot)
  semanticCandidates.push(...monorepoAgentSkillInspection.evidence, ...monorepoAgentSkillInspection.blockers)
  addIfExists(path.join(monorepoRoot, 'productivity/WAG-Content-Automation-SOP.md'))
  const dailyReportPath = path.join(monorepoRoot, 'productivity/daily-analytics/daily_report.py')
  addIfExists(dailyReportPath)
  if (fs.existsSync(dailyReportPath)) {
    const capabilities = detectPublishingCapabilities(fs.readFileSync(dailyReportPath, 'utf8'))
    semanticCandidates.push(...capabilities.map((capability) => publishingCapabilityToken(projectRoot, dailyReportPath, capability)))
  }
  addIfExists(path.join(monorepoRoot, 'productivity/growth-dashboard/COORDINATION.md'))
  const dashboardDeployPath = path.join(monorepoRoot, 'productivity/growth-dashboard/deploy-cloudflare.sh')
  addIfExists(dashboardDeployPath)
  if (fs.existsSync(dashboardDeployPath)) {
    const capabilities = detectPublishingCapabilities(fs.readFileSync(dashboardDeployPath, 'utf8'))
    semanticCandidates.push(...capabilities.map((capability) => publishingCapabilityToken(projectRoot, dashboardDeployPath, capability)))
  }
  addIfExists(path.join(monorepoRoot, 'productivity/growth-dashboard/make-deploy.sh'))
  for (const filePath of listFiles(path.join(monorepoRoot, 'productivity/seo-purge'))) {
    if (/\.(?:py|sh)$/.test(filePath)) {
      candidates.push(filePath)
      const capabilities = detectPublishingCapabilities(fs.readFileSync(filePath, 'utf8'))
      semanticCandidates.push(...capabilities.map((capability) => publishingCapabilityToken(projectRoot, filePath, capability)))
    }
  }
  ...
  for (const definition of Object.values(SCHEDULED_TASK_DEFINITIONS)) {
    ...
    if (fs.existsSync(definitionPath)) {
      ...
      const referenced = inspectReferencedEntrypoints(content, path.dirname(definitionPath))
      const capabilities = uniqueSorted([
        ...detectPublishingCapabilities(content),
        ...referenced.capabilities,
      ]) as PublishingCapability[]
      semanticCandidates.push(...capabilities.map((capability) => publishingCapabilityToken(projectRoot, definitionPath, capability)))
    }
  }

  const schedulerSnapshotPath = path.join(projectRoot, SCHEDULER_SNAPSHOT_PATH)
  if (fs.existsSync(schedulerSnapshotPath)) {
    const snapshot = parse(fs.readFileSync(schedulerSnapshotPath, 'utf8')) as ScheduledTaskSnapshot
    const actualIds = new Set((snapshot.scheduledTasks ?? []).map((task) => task.id))
    semanticCandidates.push(...[...actualIds].map(schedulerTaskToken))
    semanticCandidates.push(...SCHEDULED_TASK_IDS
      .filter((id) => !actualIds.has(id))
      .map((id) => `scheduler::missing::${id}`))
  }

  return uniqueSorted([
    ...candidates.map((candidate) => normalizeCandidatePath(projectRoot, candidate)),
    ...semanticCandidates,
  ])
}

export function findUnclassifiedPublishingCandidates(projectRoot: string): string[] {
  const { paths } = buildPublishingPaths(projectRoot)
  const classified = new Set(paths.flatMap((publishingPath) => publishingPath.evidenceFiles))
  return collectPublishingCandidates(projectRoot).filter((candidate) => !classified.has(candidate))
}

export function assertPublishingSafety(audit: SeoBaselineAudit, projectRoot: string): void {
  const blockers = [
    ...verifySchedulerSnapshotStructure(projectRoot),
    ...audit.findings.unclassifiedPublishingCandidates.map((candidate) => (
  ...
  for (const publishingPath of audit.publishingPaths) {
    if (
      ...
      blockers.push(`Unattended frontend production publication path remains capable of publishing: ${publishingPath.id}.`)
    }
    if (
      publishingPath.productionTarget === 'frontend'
      ...
      blockers.push(`Frontend publication path lacks a technical approval or hard-block control: ${publishingPath.id}.`)
    }
  }

  if (blockers.length > 0) {
    throw new Error(`SEO publishing safety audit failed:\n${uniqueSorted(blockers).map((blocker) => `- ${blocker}`).join('\n')}`)
  }
}
...
export function assertSeoReleaseSafety(
  ...
  if (blockers.length > 0) {
    throw new Error(`SEO release-safety audit failed:\n${blockers.map((blocker) => `- ${blocker}`).join('\n')}`)
  }
}

function extractArrayStringValues(source: string, variableName: string): string[] {
  const escapedName = variableName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
  const arrayMatch = source.match(new RegExp(`const\\s+${escapedName}\\s*=\\s*\\[([\\s\\S]*?)\\]`))
  if (!arrayMatch) return []
  return [...arrayMatch[1].matchAll(/['"]([^'"]+)['"]/g)].map((match) => match[1])
}

function extractLiveNavHrefs(source: string): string[] {
  return uniqueSorted([...source.matchAll(/href:\s*['"]([^'"]+)['"],\s*live:\s*true/g)].map((match) => match[1]))
}

function collectDiscoveryInventory(projectRoot: string, articles: ArticleInventoryItem[]): DiscoveryInventory {
  const blogDirectory = path.join(projectRoot, 'content/blog')
  const listingSourcePath = path.join(projectRoot, 'app/(public)/article/page.tsx')
  const listingSource = fs.existsSync(listingSourcePath) ? fs.readFileSync(listingSourcePath, 'utf8') : ''
  const slugsByImpressions = extractArrayStringValues(listingSource, 'SLUGS_BY_IMPRESSIONS')
  const impressionOrder = new Map(slugsByImpressions.map((slug, index) => [slug, index]))
  const articleListingRoutes = fs.existsSync(blogDirectory)
    ? fs.readdirSync(blogDirectory, { withFileTypes: true })
      .filter((entry) => entry.isFile() && entry.name.endsWith('.mdx'))
      .map((entry) => {
        const slug = entry.name.replace(/\.mdx$/, '')
        const raw = fs.readFileSync(path.join(blogDirectory, entry.name), 'utf8')
  ...
  const articleUtilsPath = path.join(projectRoot, 'app/(public)/article/[slug]/article-utils.ts')
  const articleUtilsSource = fs.existsSync(articleUtilsPath) ? fs.readFileSync(articleUtilsPath, 'utf8') : ''
  const blockedSlugs = new Set(extractArrayStringValues(articleUtilsSource, 'BLOCKED_SLUGS'))
  const runtimeArticleSlugs = scanMdxFilesInRuntimeOrder(blogDirectory)
    .map((filePath) => toPosixPath(path.relative(blogDirectory, filePath)).replace(/\.mdx$/, ''))
  ...
  const navSource = fs.existsSync(navSourcePath) ? fs.readFileSync(navSourcePath, 'utf8') : ''
  const liveNavHrefs = extractLiveNavHrefs(navSource)
  const byServiceBlock = navSource.match(/heading:\s*['"]nav\.menu\.byService['"][\s\S]*?links:\s*\[([\s\S]*?)\]\s*,\s*\}/)?.[1] ?? ''
  const serviceRoots = uniqueSorted([...byServiceBlock.matchAll(/href:\s*['"]([^'"]+)['"],\s*live:\s*true/g)].map((match) => match[1]))
  ...
  const globalArticleEntryPoints = ['app/components/Navbar.tsx', 'app/components/Footer.tsx']
  ...
  for (let index = 0; index < articleDetailRoutes.length - 1; index += 1) {
    ...
    recommendationEdges.push({
      ...
      relationshipCount: 1,
      source: 'app/(public)/article/[slug]/article-utils.ts::getPrevNextArticles',
    })
    recommendationEdges.push({
      mechanism: 'article-prev-next',
  ...
  const blogPreviewPath = path.join(projectRoot, 'app/components/BlogPreview.tsx')
  const blogPreviewSource = fs.existsSync(blogPreviewPath) ? fs.readFileSync(blogPreviewPath, 'utf8') : ''
  const liveSlugs = new Set(articles.map((article) => article.slug))
  ...
  if (fs.existsSync(linkGraphPath)) {
    ...
    for (const [slug, relationshipCount] of [...targetCounts.entries()].sort(([left], [right]) => compareCodePoints(left, right))) {
      recommendationEdges.push({
  ...
  recommendationEdges.sort((left, right) => compareCodePoints(
  ...
  return {
    ...
    sitemapRoutes: [...new Set([...sitemapStaticRoutes, ...sitemapArticleRoutes])],
    serviceRoots,
...
function collectFindings(
  ...
  const articleRoutes = articles.map((article) => article.route).sort(compareCodePoints)
  const nonBlankSlugs = references.flatMap((reference) => reference.slug ? [reference.slug] : [])
  const counts = new Map<string, number>()
  for (const slug of nonBlankSlugs) counts.set(slug, (counts.get(slug) ?? 0) + 1)
  const referencedSlugs = new Set(nonBlankSlugs)

  return {
    missingPublishedLegacySlugs: [...new Set(references
      .filter((reference) => reference.declaredStatus === 'published' && reference.inventoryStatus === 'missing')
      .flatMap((reference) => reference.slug ? [reference.slug] : []))]
      .sort(compareCodePoints),
    retiredLegacySlugs: [...new Set(references
      .filter((reference) => reference.inventoryStatus === 'gone-410')
      .flatMap((reference) => reference.slug ? [reference.slug] : []))]
      .sort(compareCodePoints),
    missingLegacySlugs: [...new Set(references
    ...
    articlesMissingFrontmatterSlug: articles
      ...
      .map((article) => article.slug)
      .sort(compareCodePoints),
    plannedLegacyPlaceholders: references.filter((reference) => reference.inventoryStatus === 'planned-placeholder').length,
    articleRoutesMissingFromListing: articleRoutes.filter((route) => !discoveryInventory.articleListingRoutes.includes(route)),
    articleRoutesMissingFromDetail: articleRoutes.filter((route) => !discoveryInventory.articleDetailRoutes.includes(route)),
    articleRoutesMissingFromSitemap: articleRoutes.filter((route) => !discoveryInventory.sitemapArticleRoutes.includes(route)),
    duplicateCanonicals: [...articles.reduce<Map<string, string[]>>((groups, article) => {
      groups.set(article.canonical, [...(groups.get(article.canonical) ?? []), article.route])
      return groups
    }, new Map()).entries()]
      .filter(([, routes]) => routes.length > 1)
      .map(([canonical, routes]) => `${canonical} => ${routes.sort(compareCodePoints).join(', ')}`)
      .sort(compareCodePoints),
    staleArticleLinks: uniqueSorted(articles.flatMap((article) => article.internalLinks)
      .map((link) => link.split(/[?#]/, 1)[0].replace(/\/$/, ''))
      .filter((link) => link.startsWith('/article/') && link !== '/article/faq' && !articleRoutes.includes(link))),
    articlesWithoutBodyInboundLinks: articleRoutes.filter((route) => !articles.some((article) => article.internalLinks
      .map((link) => link.split(/[?#]/, 1)[0].replace(/\/$/, ''))
      .includes(route))),
    configuredRecommendationTargetsMissing: (() => {
      const blogPreviewPath = path.join(projectRoot, 'app/components/BlogPreview.tsx')
      const source = fs.existsSync(blogPreviewPath) ? fs.readFileSync(blogPreviewPath, 'utf8') : ''
      const liveSlugs = new Set(articles.map((article) => article.slug))
...
export function buildSeoBaselineAudit(projectRoot: string): SeoBaselineAudit {
  ...
  const unclassifiedPublishingCandidates = findUnclassifiedPublishingCandidates(projectRoot)
...
export function renderBaselineMarkdown(audit: SeoBaselineAudit): string {
  const articleRows = audit.articles.map((article) => [
    `\`${article.slug}\``,
  ...
  const clusterRows = audit.legacyClusters.map((cluster) => {
    const live = cluster.references.filter((reference) => reference.inventoryStatus === 'live').length
    const gone = cluster.references.filter((reference) => reference.inventoryStatus === 'gone-410').length
    const missing = cluster.references.filter((reference) => reference.inventoryStatus === 'missing').length
    const planned = cluster.references.filter((reference) => reference.inventoryStatus === 'planned-placeholder').length
    return `\`${cluster.path}\` | ${cluster.references.length} | ${live} | ${gone} | ${missing} | ${planned}`
  }).join('\n')

  const legacyReferenceRows = audit.legacyClusters.flatMap((cluster) => cluster.references.map((reference) => [
    `\`${cluster.path}\``,
    reference.role,
    reference.type,
  ...
  const discoveryRows = audit.discoverySurfaces.map((surface) => [
    surface.id,
    `\`${surface.source}\``,
  ...
  const discoveryCoverageRows = [
    ...
    item.missing.length === 0 ? 'None' : item.missing.map((route) => `\`${route}\``).join(', '),
  ].join(' | ')).join('\n')

  const recommendationRows = audit.discoveryInventory.recommendationEdges.map((edge) => [
    edge.mechanism,
    `\`${edge.sourceRoute}\``,
    `\`${edge.targetRoute}\``,
  ...
  const measurementRows = audit.measurements.map((measurement) => [
  ...
  const publishingRows = audit.publishingPaths.map((publishingPath) => [
    publishingPath.id,
    publishingPath.kind,
    ...
    publishingPath.evidenceFiles.map((file) => `\`${file}\``).join(', ') || 'None',
    escapeTableCell(publishingPath.hardBlocker),
  ].join(' | ')).join('\n')

  const enabledUnattendedFrontendDeploys = audit.publishingPaths.filter((item) => (
  ...
  const unguardedNotificationPaths = audit.publishingPaths.filter((item) => (
    item.kind === 'notification'
    && item.observedState === 'enabled'
    && item.approvalEnforcement === 'none'
  )).length

  const enabledUnattendedDashboardDeploys = audit.publishingPaths.filter((item) => (
    item.classification === 'unattended'
    && item.observedState === 'enabled'
  ...
  const manualFrontendProductionPaths = audit.publishingPaths.filter((item) => (
