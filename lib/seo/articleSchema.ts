...
const GOVERNED_FIELDS: Array<keyof z.infer<typeof articleGovernedFieldsSchema>> = [
  'contentId',
  'cluster',
  'contentRole',
  'searchIntent',
...
export function parseArticleFrontmatter(
  ...
  // Always pass through optional arrays/keywords if present on the raw object.
  for (const field of [
    'secondaryKeywords',
  ...
  if (mode === 'strict') {
    ...
    for (const field of GOVERNED_FIELDS) {
      if (!isPresent(governedResult.data[field])) {
        missing.push(field)
      }
    }
    if (missing.length > 0) {
  ...
  const contentId =
    governedResult.data.contentId ?? deriveContentId(fileSlug)
