...
describe('parseArticleFrontmatter', () => {
  it('accepts a fully governed article without warnings', () => {
    ...
    expect(result.frontmatter.contentId).toBe('article.check-chinese-company-samr')
  ...
  it('accepts legacy display-only frontmatter with deterministic governed warnings', () => {
    ...
    expect(first.warnings.every((w) => w.articleId === 'check-chinese-company-samr')).toBe(
      true,
    )
    expect(first.warnings.some((w) => w.code === 'missing_governed_field')).toBe(true)
  ...
  it('uses legacy desc as description and records an alias warning', () => {
    ...
    expect(result.frontmatter.description).toBe('Legacy summary used as description.')
    expect(
  ...
  it('warns when frontmatter slug disagrees with the filename slug', () => {
    ...
    expect(
      result.warnings.some((w) => w.code === 'slug_mismatch' && w.field === 'slug'),
    ).toBe(true)
  })
})
