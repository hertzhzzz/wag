...
const actionLabels: Record<string, string> = {
  "access-page": "Page", "dashboard": "Dashboard", "report-view": "Report",
}
...
export default async function AdminClientsPage() {
  ...
  // Dynamic import — avoids RSC module-level crash from crypto/Redis imports
  const { validateAdminSession } = await import("@/lib/admin-auth")
  if (!(await validateAdminSession(sessionCookie.value))) { redirect("/admin") }
  ...
  if (redis) {
    for (const c of clients.slice(0, 5)) {
      try { const e = await redis.lrange(`access_logs:${c.slug}`, 0, 9); for (const item of e) { try { recentActivity.push({ ...(JSON.parse(item as string) as AccessLogEntry), client_name: c.client_name }) } catch { /* */ } } } catch { /* */ }
    }
    recentActivity.sort((a, b) => b.timestamp.localeCompare(a.timestamp)); recentActivity = recentActivity.slice(0, 20)
  }
