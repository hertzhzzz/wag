...
export default async function AdminClientDetailPage({
  ...
  if (redis) {
    ...
    activeSessions = await redis.scard(`client_sessions:${slug}`)
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <Link href="/admin/clients" className="text-xs text-gray-400 hover:text-navy transition mb-1 inline-block">
            &larr; All Clients
          </Link>
        ...
        <div className="flex items-center gap-4">
          <StatBadge label="Active Sessions" value={String(activeSessions)} />
          <StatBadge label="Total Views" value={String(logs.length)} />
          <StatBadge label="Reports" value={String(Object.keys(reportViewCounts).length)} />
        </div>
      </div>

      {Object.keys(reportViewCounts).length > 0 && (
        <section className="mb-8">
          <h2 className="text-base font-semibold text-navy mb-3">Report Views</h2>
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <div className="space-y-2">
              {Object.entries(reportViewCounts)
                .sort(([, a], [, b]) => b - a)
                .map(([id, count]) => (
                  <div key={id} className="flex items-center justify-between text-sm">
                    <span className="text-gray-700 font-mono text-xs">{id}</span>
                    <span className="font-semibold text-navy">{count} views</span>
                  </div>
                ))}
            </div>
          </div>
      ...
      <section>
        <h2 className="text-base font-semibold text-navy mb-3">Access Log</h2>
        {logs.length === 0 ? (
          <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
            <p className="text-gray-400 text-sm">No access records yet.</p>
          </div>
        ) : (
